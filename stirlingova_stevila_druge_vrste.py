...
def  fakulteta(n):
    # nimam srca, da bi to naredil z rekurzijo
    ans = 1
    for i in range(n):
        ans *= i + 1
    return int(ans)
...
def stirling_vsota(n, k, j=0):
    ...
    if j == 0:
        return k ** n
    return stirling_vsota(n, k , j-1) + ((-1) ** j) * binomski_ucinkovit(k, j) * (k - j) ** n
# =====================================================================@020027=
# 3. podnaloga
...
def stirling_direktno(n, k):
    return ( (1) * stirling_vsota(n, k, k)) // fakulteta(k)
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzYzLCJwYXJ0IjoyMDAyM30:1jAzlQ:hQ81U1yxgqQTWwYXDU38fYkYgek'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzYzLCJwYXJ0IjoyMDAyNH0:1jAzlQ:jVe5l3JXYy_0-z8_z6NCuhjZh58'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzYzLCJwYXJ0IjoyMDAyN30:1jAzlQ:si3WhaV4hDbM-Qi2buzM1bPt2Jw'
        try:
