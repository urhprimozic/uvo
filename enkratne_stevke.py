...
def kolikokrat_se_pojavi_stevka(k, n):
    #če je n samo ena števka, je očitno
    if n < 10:
        if k == n:
            return 1
        return 0
    stevka = n % 10
    return int(stevka == k) + kolikokrat_se_pojavi_stevka(k, n // 10)
...
def ali_ima_enkratne_stevke(n):
    # če je številka dovolj velika se nam ne spača računatio
    if n > 976543210:
        return False
    # v primeru ene števke je očitno
    if n < 10:
        return True
    # drugače 
    a = [False] * 10
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzYzLCJwYXJ0IjoyMDAyNX0:1j6J9s:pif9nEn72hvnEBsjzpFJq-59QRY'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzYzLCJwYXJ0IjoyMDAyNn0:1j6J9s:-44MsqX9A6w0XZLv11xUji5LRjg'
        try:
