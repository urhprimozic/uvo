...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzYzLCJwYXJ0Ijo5NzczfQ:1jBlMD:28P-XhF_T33BtRA0CiG-HpBL6Bo'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzYzLCJwYXJ0Ijo5Nzc0fQ:1jBlMD:z_DqfJ3SkxIqmDAhAIK1FVrqI0U'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzYzLCJwYXJ0Ijo5NzcyfQ:1jBlMD:KSXtTXfyPqhF9visb6nQDU__usA'
        try:
            Check.equal('odstrani_presledke("Ni vsak dan nedelja")', 'Nivsakdannedelja')
            Check.equal('odstrani_presledke("M a t i j a")', 'Matija')
            Check.equal('odstrani_presledke(" Pre šer   en _  je  _precej  _ kul    ")', 'Prešeren_je_precej_kul')
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzYzLCJwYXJ0IjoyMDExNX0:1jBlMD:-AnoI1QYZa_xmRXSBRvTQkE8YUA'
        try:
            Check.equal('odstrani_ponovljene_presledke("  * -   *   - * ")', ' * - * - * ')
            Check.equal('odstrani_ponovljene_presledke("M  a    t  i    j   a")', 'M a t i j a')
            Check.equal('odstrani_ponovljene_presledke("  ovca  ")', ' ovca ')
            Check.equal('odstrani_ponovljene_presledke("      ")', ' ')
            Check.equal('odstrani_ponovljene_presledke("")', '')
            Check.equal('odstrani_ponovljene_presledke(" ")', ' ')
        except:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjo1MzYzLCJwYXJ0IjoyMDExNH0:1jBlMD:XlYlSpKGgtD_vSoRk9FU6wiBmX4'
        try:
            Check.equal('najdi_stevilko("05123***6", ["041890343", "051234446", "051342236"])', ["051234446"])
            Check.equal('najdi_stevilko("0********", ["041890343", "051234446", "051342236"])', ["041890343", "051234446", "051342236"])
            Check.equal('najdi_stevilko("0*1123*57", ["041123457", "071123456", "051123457"])', ["041123457", "051123457"])
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
    ...
    try:
        ...
        token = 'Token c9e65283f2347f61e1c913416d3e32579b71e9ab'
